//******************************************************************************
//  Demo Application07 for MSP430/LMP90100 Interface Code Library v1.0
//  DRDYB multiplexed with SDO instead of on D6 pin. SDO/DRDYB should be connected
//  to both SOMI (P3.4) and INT (P2.4) pins as shown below
//  Normal Stream Read ADC Data & status (2 channels) with interrupt and CRC Check
//
//                MSP430F5528
//             -----------------
//         /|\|              XIN|-
...
void main(void)
{
  ...
  while (1)
  {
    if (LMP90100_ADCDataReady)
    {
      ...
      TI_LMP90100_SPINormalStreamReadADC(addr, read_buf, count, &prev_URA);    // read status, adc output & CRC into read_buf
