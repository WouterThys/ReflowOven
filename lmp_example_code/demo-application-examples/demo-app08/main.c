//******************************************************************************
//  Demo Application08 for MSP430/LMP90100 Interface Code Library v1.0
//  Data Only Read ADC Data & status (2 channels) with interrupt and CRC Check
//
//                MSP430F5528
//             -----------------
//         /|\|              XIN|-
...
#define NUM_ACTIVE_CHANNELS 2                                                  // Number of Active Channels
#define ERROR_THRESHOLD   8                                                    // number of crc errors above which sampling loop is exited
//******************************************************************************
void main(void)
{
  ...
  uint32_t adc_data, adc_sample_array[NUM_ACTIVE_CHANNELS][SAMPLE_ARRAY_SIZE];
  uint8_t ch_status, ch_num, ch_index[NUM_ACTIVE_CHANNELS];
  ...
  addr = TI_LMP90100_SENDIAG_FLAGS_REG;                                        // Start Reading from SENDIAG_FLAGS Register
  count = CH_DATA_SIZE;                                                        // bytes to read: status, ADC_DOUT2 - ADCDOUT0, CRC
  for(i=0; i<NUM_ACTIVE_CHANNELS; i++)                                         // initialize ch index of all active channels
    ch_index[i] = 0;
 
  TI_LMP90100_SPIEnableDataFirstMode(addr, count, &prev_URA);                  // Data Frist Mode Enable
  while (1)
  { 
    if (LMP90100_ADCDataReady)
    {
      LMP90100_ADCDataReady = 0;                                               // clear flag
      TI_LMP90100_SPIDataOnlyReadADC(read_buf, count);                         // read status, adc output & CRC into read_buf
      ...
      {
        i = ch_index[ch_num];
        adc_sample_array[ch_num][i] = adc_data;
        ch_index[ch_num]++;
        if (ch_index[ch_num] == SAMPLE_ARRAY_SIZE)                             // sample array is full
        {
          process_adc_output(adc_sample_array[ch_num]);                        // dummy app function: toggles led
          ch_index[ch_num] = 0;
          error_count = 0;
        }
      } else
  ...
  TI_LMP90100_SPIDisableDataFirstMode(read_buf, count);                        // demonstrate data first mode disable
  __no_operation();                                                            // For debugger  
}
//******************************************************************************
...
void TI_LMP90100_WriteRegSettings(uint8_t *pURA)
{
  ...
  TI_LMP90100_SPIWriteReg(TI_LMP90100_CH_SCAN_REG, 
                          TI_LMP90100_CH_SCAN_REG_VALUE, pURA);                // Two channels continuous scan, ch# = 0, 1   
  // CH0 in use
