...
//******************************************************************************
#if TI_LMP90100_SER_INTF == TI_LMP90100_SER_INTF_USCIA0_5xx
...
void TI_LMP90100_SPISetup(void)
{
  ...
  UCA0BR1 = 0;
  UCA0MCTL = 0;
  TI_LMP90100_SPI_USCIA0_PxSEL1 |= TI_LMP90100_SPI_USCIA0_SIMO
                                   | TI_LMP90100_SPI_USCIA0_SOMI;              
  TI_LMP90100_SPI_USCIA0_PxSEL2 |= TI_LMP90100_SPI_USCIA0_UCLK;
                                                                               // SPI option select
  TI_LMP90100_SPI_USCIA0_PxDIR1 |= TI_LMP90100_SPI_USCIA0_SIMO;
  TI_LMP90100_SPI_USCIA0_PxDIR2 |= TI_LMP90100_SPI_USCIA0_UCLK;
...
void TI_LMP90100_SPIEnableDataFirstMode(uint8_t addr, uint8_t count, uint8_t *pURA)
{
  ...
  while (!(UCA0IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA0TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_ENABLE;                     // write Data First mode instruction
  
  while (!(UCA0IFG&UCTXIFG));                                                  // Wait for TXBUF ready  
  UCA0TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write read mode status instruction
  while (UCA0STAT & UCBUSY);                                                   // Wait for TX complete
  while (!(UCA0RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG))                  // wait for device to enter data first mode
  {
    UCA0TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;         // Write device read mode status instruction
    while (UCA0STAT & UCBUSY);                                                 // Wait for TX complete
  }
...
void TI_LMP90100_SPIDisableDataFirstMode(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    UCA0TXBUF =0;                                                              // Dummy write so we can read data     
    while (UCA0STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCA0TXBUF;                                                   // Read register data    
  }
  
  while (!(UCA0IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA0TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_DISABLE;                    // Disable Data First Mode
  while (!(UCA0IFG&UCTXIFG));                                                  // Wait for TXBUF ready    
  UCA0TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write device read mode status instruction
  while (UCA0STAT & UCBUSY);                                                   // Wait for TX complete
  while (UCA0RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG)                     // wait for device to exit data first mode
  {
    UCA0TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;         // Write device read mode status instruction
    while (UCA0STAT & UCBUSY);                                                 // Wait for TX complete
  }
