...
//******************************************************************************
// Support for 552x USART1
//******************************************************************************
#if TI_LMP90100_SER_INTF == TI_LMP90100_SER_INTF_USART1
...
void TI_LMP90100_SPISetup(void)
{
  ...
  ME2 |= USPIE1;                                                               // Enable USART0 SPI mode
  ...
  TI_LMP90100_SPI_USART1_PxSEL |= TI_LMP90100_SPI_USART1_SIMO
                          | TI_LMP90100_SPI_USART1_SOMI
                          | TI_LMP90100_SPI_USART1_UCLK;                       // SPI option select
  TI_LMP90100_SPI_USART1_PxDIR |= TI_LMP90100_SPI_USART1_SIMO 
                                  + TI_LMP90100_SPI_USART1_UCLK;               // SPI TX out direction  
...
void TI_LMP90100_SPIWriteReg(uint8_t addr, uint8_t value, uint8_t *pURA)
{
  ...
  while (TI_LMP90100_SPI_USART1_PxIN & TI_LMP90100_SPI_USART1_SOMI);           // wait for LMP90100 ready
  ...
  {    
    ...
    U1TXBUF = inst;                                                            // Send instruction
    while (!(IFG2 & URXIFG1));                                                 // wait for tx to finish
    
    while (!(IFG2 & UTXIFG1));                                                 // Wait for TXBUF ready
    IFG2 &= ~URXIFG1;                                                          // Clear flag 
    U1TXBUF = new_URA;                                                         // Send upper register address
    while (!(IFG2 & URXIFG1));                                                 // wait for tx to finish
  ...
  while (!(IFG2 & UTXIFG1))                                                    // Wait for TXBUF ready
  IFG2 &= ~URXIFG1;                                                            // Clear flag 
  U1TXBUF = inst;                                                              // Send lower register address
  while (!(IFG2 & URXIFG1));                                                   // wait for tx to finish  

  while (!(IFG2&UTXIFG1));                                                     // Wait for TXBUF ready
  IFG2 &= ~URXIFG1;                                                            // Clear flag 
  U1TXBUF = value;                                                             // Send data value
  while (!(IFG2 & URXIFG1));                                                   // wait for tx to finish  
                                         ...
                                         uint8_t count, uint8_t *pURA)
{
  ...
  for(i= 0; i < count; i++)
  {
    while (!(IFG2 & UTXIFG1));                                                 // Wait for TXBUF ready
    IFG2 &= ~URXIFG1;                                                          // Clear flag   
    U1TXBUF = *(buffer+i);                                                     // Send data value  
    while (!(IFG2 & URXIFG1));                                                 // wait for tx to finish    
  }
...
void TI_LMP90100_SPIEnableDataFirstMode(uint8_t addr, uint8_t count, uint8_t *pURA)
{
  ...
  while (!(IFG2 & UTXIFG1));                                                   // Wait for TXBUF ready
  IFG2 &= ~URXIFG1;                                                            // clear flag
  U1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_ENABLE;                       // write Data First mode instruction
  while (!(IFG2 & URXIFG1));                                                   // wait for tx to finish
  
  while (!(IFG2 & UTXIFG1));                                                   // Wait for TXBUF ready 
  IFG2 &= ~URXIFG1;                                                            // clear flag
  U1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;             // Write read mode status instruction
  while (!(IFG2 & URXIFG1));                                                   // wait for tx to finish
  
  while (!(U1RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG))                    // wait for device to enter data first mode
  {
    IFG2 &= ~URXIFG1;                                                          // clear flag
    U1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write read mode status instruction
    while (!(IFG2 & URXIFG1));                                                 // wait for tx to finish
...
void TI_LMP90100_SPIDisableDataFirstMode(uint8_t *buffer, uint8_t count)
{
  ...
  while (!(IFG2 & UTXIFG1));                                                   // Wait for TXBUF ready
  IFG2 &= ~URXIFG1;                                                            // clear flag
  U1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_DISABLE;                      // Disable Data First Mode
  while (!(IFG2 & URXIFG1));                                                   // wait for tx to finish 
  
  while (!(IFG2 & UTXIFG1));                                                   // Wait for TXBUF ready 
  IFG2 &= ~URXIFG1;                                                            // clear flag  
  U1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;             // Write device read mode status instruction
  while (!(IFG2 & URXIFG1));                                                   // wait for tx to finish 
  
  while (U1RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG)                       // wait for device to exit data first mode
  {
    IFG2 &= ~URXIFG1;                                                          // clear flag
    U1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write read mode status instruction
    while (!(IFG2 & URXIFG1));                                                 // wait for tx to finish
