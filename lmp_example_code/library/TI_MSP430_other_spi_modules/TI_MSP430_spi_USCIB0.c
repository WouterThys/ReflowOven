...
//******************************************************************************
#if TI_LMP90100_SER_INTF == TI_LMP90100_SER_INTF_USCIB0
...
void TI_LMP90100_SPISetup(void)
{
  ...
  UCB0BR0 = 0x04;                                                              // UCLK/4
  UCB0BR1 = 0;
  TI_LMP90100_SPI_UCB0_PxSEL1 |= TI_LMP90100_SPI_UCB0_SIMO
                                   | TI_LMP90100_SPI_UCB0_SOMI;              
  TI_LMP90100_SPI_UCB0_PxSEL2 |= TI_LMP90100_SPI_UCB0_UCLK;
                                                                               // SPI option select
  TI_LMP90100_SPI_UCB0_PxDIR1 |= TI_LMP90100_SPI_UCB0_SIMO;
  TI_LMP90100_SPI_UCB0_PxDIR2 |= TI_LMP90100_SPI_UCB0_UCLK;
...
void TI_LMP90100_SPIDataOnlyReadADC(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    while (!(UC1IFG&UCB0TXIFG));                                               // Wait for TXBUF ready
    UCB0TXBUF =0;                                                              // Dummy write so we can read data     
    while (UCB0STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCB0RXBUF;                                                   // Read register data    
  }
...
void TI_LMP90100_SPIEnableDataFirstMode(uint8_t addr, uint8_t count, uint8_t *pURA)
{
  ...
  {
    UCB0TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;         // Dummy write so we can read data
    while (UCB0STAT & UCBUSY);                                                 // Wait for TX complete
  }
