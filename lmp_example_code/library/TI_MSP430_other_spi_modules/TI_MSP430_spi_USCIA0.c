...
//******************************************************************************
#if TI_LMP90100_SER_INTF == TI_LMP90100_SER_INTF_USCIA0
//------------------------------------------------------------------------------
...
void TI_LMP90100_SPISetup(void)
{
  ...
  UCA0BR1 = 0;
  UCA0MCTL = 0;
  TI_LMP90100_SPI_UCA0_PxSEL1 |= TI_LMP90100_SPI_UCA0_SIMO
                                   | TI_LMP90100_SPI_UCA0_SOMI;              
  TI_LMP90100_SPI_UCA0_PxSEL2 |= TI_LMP90100_SPI_UCA0_UCLK;
                                                                               // SPI option select
  TI_LMP90100_SPI_UCA0_PxDIR1 |= TI_LMP90100_SPI_UCA0_SIMO;
  TI_LMP90100_SPI_UCA0_PxDIR2 |= TI_LMP90100_SPI_UCA0_UCLK;
...
void TI_LMP90100_SPIDataOnlyReadADC(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    while (!(UC1IFG&UCA0TXIFG));                                               // Wait for TXBUF ready
    UCA0TXBUF =0;                                                              // Dummy write so we can read data     
    while (UCA0STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCA0RXBUF;                                                   // Read register data    
  }
