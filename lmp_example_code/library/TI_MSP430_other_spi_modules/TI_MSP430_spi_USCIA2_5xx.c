...
//******************************************************************************
#if TI_LMP90100_SER_INTF == TI_LMP90100_SER_INTF_USCIA2_5xx
...
void TI_LMP90100_SPISetup(void)
{
  ...
  UCA2BR0 = 0x04;                                                              // UCLK/4
  UCA2BR1 = 0;
  TI_LMP90100_SPI_USCIA2_PxSEL1 |= TI_LMP90100_SPI_USCIA2_SIMO
                                   | TI_LMP90100_SPI_USCIA2_SOMI;              
  TI_LMP90100_SPI_USCIA2_PxSEL2 |= TI_LMP90100_SPI_USCIA2_UCLK;
                                                                               // SPI option select
  TI_LMP90100_SPI_USCIA2_PxDIR1 |= TI_LMP90100_SPI_USCIA2_SIMO;
  TI_LMP90100_SPI_USCIA2_PxDIR2 |= TI_LMP90100_SPI_USCIA2_UCLK;
...
void TI_LMP90100_SPIWriteReg(uint8_t addr, uint8_t value, uint8_t *pURA)
{
  ...
  {    
    ...
    UCA2TXBUF = inst;                                                          // Send instruction
  
    while (!(UCA2IFG&UCTXIFG));                                                // Wait for TXBUF ready
    UCA2TXBUF = new_URA;                                                       // Send upper register address 
...
void TI_LMP90100_SPIEnableDataFirstMode(uint8_t addr, uint8_t count, uint8_t *pURA)
{
  ...
  while (!(UCA2IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA2TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_ENABLE;                     // write Data First mode instruction
  
  while (!(UCA2IFG&UCTXIFG));                                                  // Wait for TXBUF ready  
  UCA2TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write read mode status instruction
  while (UCA2STAT & UCBUSY);                                                   // Wait for TX complete
  while (!(UCA2RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG))                  // wait for device to enter data first mode
  {
    UCA2TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;         // Write device read mode status instruction
    while (UCA2STAT & UCBUSY);                                                 // Wait for TX complete
  }
...
void TI_LMP90100_SPIDisableDataFirstMode(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    UCA2TXBUF =0;                                                              // Dummy write so we can read data     
    while (UCA2STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCA2TXBUF;                                                   // Read register data    
  }
  
  while (!(UCA2IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA2TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_DISABLE;                    // Disable Data First Mode
  while (!(UCA2IFG&UCTXIFG));                                                  // Wait for TXBUF ready    
  UCA2TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write device read mode status instruction
  while (UCA2STAT & UCBUSY);                                                   // Wait for TX complete
  while (UCA2RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG)                     // wait for device to exit data first mode
  {
    UCA2TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;         // Write device read mode status instruction
    while (UCA2STAT & UCBUSY);                                                 // Wait for TX complete
  }
