...
//******************************************************************************
#if TI_LMP90100_SER_INTF == TI_LMP90100_SER_INTF_USI
...
void TI_LMP90100_SPISetup(void)
{
  ...
  USICNT = 1;                                                                  // to avoid conflict with LMP90100
}
...
void TI_LMP90100_SPIWriteReg(uint8_t addr, uint8_t value, uint8_t *pURA)
{
  ...
  {    
    ...
    USISRL = inst;                                                             // Load instruction
    USICNT = 8;                                                                // Send it
    while (!(USICTL1&USIIFG));                                                 // Wait for TX to finish    
    
  
    USISRL = new_URA;                                                          // Load upper register address
    USICNT = 8;                                                                // Send it
    while (!(USICTL1&USIIFG));                                                 // Wait for TX to finish  
  ...
  USISRL = inst;                                                               // Load lower register address
  USICNT = 8;                                                                  // Send it
  while (!(USICTL1&USIIFG));                                                   // Wait for TX to finish 
  
  USISRL = value;                                                              // Load data value
  USICNT = 8;                                                                  // Send it
  while (!(USICTL1&USIIFG));                                                   // Wait for TX to finish  
                                         ...
                                         uint8_t count, uint8_t *pURA)
{
  ...
  for(i= 0; i < count; i++)
  {
    USISRL = *(buffer+i);                                                      // Load data value
    USICNT = 8;                                                                // Send it
    while (!(USICTL1&USIIFG));                                                 // Wait for TX to finish    
  }
...
void TI_LMP90100_SPIEnableDataFirstMode(uint8_t addr, uint8_t count, uint8_t *pURA)
{
  ...
  USISRL = LMP90100_DATA_FIRST_MODE_INSTRUCTION_ENABLE;                        // Load Data First mode instruction
  USICNT = 8;                                                                  // Send it
  while (!(USICTL1&USIIFG));                                                   // Wait for TX to finish

  USISRL = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;              // Load read mode status instruction
  USICNT = 8;                                                                  // Send it
  while (!(USICTL1&USIIFG));                                                   // Wait for TX to finish
  
  while (!(USISRL & LMP90100_DATA_FIRST_MODE_STATUS_FLAG))                     // wait for device to enter data first mode
  {
    USICNT = 8;                                                                // dummy write so we can read data
    while (!(USICTL1&USIIFG));                                                 // Wait for TX to finish
  }
...
void TI_LMP90100_SPIDisableDataFirstMode(uint8_t *buffer, uint8_t count)
{
  ...
  USISRL = LMP90100_DATA_FIRST_MODE_INSTRUCTION_DISABLE;                       // Load Data First mode instruction
  USICNT = 8;                                                                  // Send it
  while (!(USICTL1&USIIFG));                                                   // Wait for TX to finish
  ...
  while (!(USISRL & LMP90100_DATA_FIRST_MODE_STATUS_FLAG))                     // wait for device to exit data first mode
  {
    USICNT = 8;                                                                // dummy write so we can read data
    while (!(USICTL1&USIIFG));                                                 // Wait for TX to finish
  }
