...
//******************************************************************************
#if TI_LMP90100_SER_INTF == TI_LMP90100_SER_INTF_USCIA3_5xx
...
void TI_LMP90100_SPISetup(void)
{
  ...
  UCA3BR0 = 0x04;                                                              // UCLK/4
  UCA3BR1 = 0;
  TI_LMP90100_SPI_USCIA3_PxSEL1 |= TI_LMP90100_SPI_USCIA3_SIMO
                                   | TI_LMP90100_SPI_USCIA3_SOMI;              
  TI_LMP90100_SPI_USCIA3_PxSEL2 |= TI_LMP90100_SPI_USCIA3_UCLK;
                                                                               // SPI option select
  TI_LMP90100_SPI_USCIA3_PxDIR1 |= TI_LMP90100_SPI_USCIA3_SIMO;
  TI_LMP90100_SPI_USCIA3_PxDIR2 |= TI_LMP90100_SPI_USCIA3_UCLK;
...
void TI_LMP90100_SPIWriteReg(uint8_t addr, uint8_t value, uint8_t *pURA)
{
  ...
  {    
    ...
    UCA3TXBUF = inst;                                                          // Send instruction
  
    while (!(UCA3IFG&UCTXIFG));                                                // Wait for TXBUF ready
    UCA3TXBUF = new_URA;                                                       // Send upper register address 
...
void TI_LMP90100_SPIEnableDataFirstMode(uint8_t addr, uint8_t count, uint8_t *pURA)
{
  ...
  while (!(UCA3IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA3TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_ENABLE;                     // write Data First mode instruction
  
  while (!(UCA3IFG&UCTXIFG));                                                  // Wait for TXBUF ready  
  UCA3TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write read mode status instruction
  while (UCA3STAT & UCBUSY);                                                   // Wait for TX complete
  while (!(UCA3RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG))                  // wait for device to enter data first mode
  {
    UCA3TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;         // Write device read mode status instruction
    while (UCA3STAT & UCBUSY);                                                 // Wait for TX complete
  }
...
void TI_LMP90100_SPIDisableDataFirstMode(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    UCA3TXBUF =0;                                                              // Dummy write so we can read data     
    while (UCA3STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCA3TXBUF;                                                   // Read register data    
  }
  
  while (!(UCA3IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA3TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_DISABLE;                    // Disable Data First Mode
  while (!(UCA3IFG&UCTXIFG));                                                  // Wait for TXBUF ready    
  UCA3TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write device read mode status instruction
  while (UCA3STAT & UCBUSY);                                                   // Wait for TX complete
  while (UCA3RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG)                     // wait for device to exit data first mode
  {
    UCA3TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;         // Write device read mode status instruction
    while (UCA3STAT & UCBUSY);                                                 // Wait for TX complete
  }
