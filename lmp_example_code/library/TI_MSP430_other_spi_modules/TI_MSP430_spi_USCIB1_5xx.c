...
//******************************************************************************
#if TI_LMP90100_SER_INTF == TI_LMP90100_SER_INTF_USCIB1_5xx
...
void TI_LMP90100_SPISetup(void)
{
  ...
  UCB1BR0 = 0x04;                                                              // UCLK/4
  UCB1BR1 = 0;
  TI_LMP90100_SPI_USCIB1_PxSEL1 |= TI_LMP90100_SPI_USCIB1_SIMO
                                   | TI_LMP90100_SPI_USCIB1_SOMI;              
  TI_LMP90100_SPI_USCIB1_PxSEL2 |= TI_LMP90100_SPI_USCIB1_UCLK;
                                                                               // SPI option select
  TI_LMP90100_SPI_USCIB1_PxDIR1 |= TI_LMP90100_SPI_USCIB1_SIMO;
  TI_LMP90100_SPI_USCIB1_PxDIR2 |= TI_LMP90100_SPI_USCIB1_UCLK;
...
void TI_LMP90100_SPIWriteReg(uint8_t addr, uint8_t value, uint8_t *pURA)
{
  ...
  {    
    ...
    UCB1TXBUF = inst;                                                          // Send instruction
  
    while (!(UCB1IFG&UCTXIFG));                                                // Wait for TXBUF ready
    UCB1TXBUF = new_URA;                                                       // Send upper register address 
...
void TI_LMP90100_SPIEnableDataFirstMode(uint8_t addr, uint8_t count, uint8_t *pURA)
{
  ...
  while (!(UCB1IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCB1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_ENABLE;                     // write Data First mode instruction
  
  while (!(UCB1IFG&UCTXIFG));                                                  // Wait for TXBUF ready  
  UCB1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write read mode status instruction
  while (UCB1STAT & UCBUSY);                                                   // Wait for TX complete  
  while (!(UCB1RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG))                  // wait for device to enter data first mode
  {
    UCB1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;         // Write device read mode status instruction
    while (UCB1STAT & UCBUSY);                                                 // Wait for TX complete
  } 
...
void TI_LMP90100_SPIDisableDataFirstMode(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    UCB1TXBUF =0;                                                              // Dummy write so we can read data     
    while (UCB1STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCB1TXBUF;                                                   // Read register data    
  }
  
  while (!(UCB1IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCB1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_DISABLE;                    // Disable Data First Mode
  while (!(UCB1IFG&UCTXIFG));                                                  // Wait for TXBUF ready    
  UCB1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;           // Write device read mode status instruction
  while (UCB1STAT & UCBUSY);                                                   // Wait for TX complete  
  while (UCB1RXBUF & LMP90100_DATA_FIRST_MODE_STATUS_FLAG)                     // wait for device to exit data first mode
  {
    UCB1TXBUF = LMP90100_DATA_FIRST_MODE_INSTRUCTION_READ_MODE_STATUS;         // Write device read mode status instruction
    while (UCB1STAT & UCBUSY);                                                 // Wait for TX complete
  }
